...
describe("Historical Comparison Gate", () => {
  ...
  it("rejects an empty provider and warehouse comparison", () => {
    const artifact = buildHistoricalComparisonArtifact({
      apiRows: [],
      provider: "google_ads",
      warehouseRows: [],
    ...
    expect(artifact.dataIssues).toStrictEqual([
      {
  ...
  it("normalizes database Date values without local timezone drift", () => {
    expect(
      normalizeWarehouseCampaignDailyRow({
        campaign_id: "campaign-1",
        date_day: new Date("2026-06-22T00:00:00.000Z"),
      }).date
    ).toBe("2026-06-22");
  });

  it("rejects invalid warehouse date values", () => {
    expect(() =>
      normalizeWarehouseCampaignDailyRow({
        campaign_id: "campaign-1",
        date_day: "2026-02-30",
      })
    ).toThrow(/exact date_day/u);
  });

  it("rejects database Date values outside four-digit years", () => {
    const date = new Date("2026-01-01T00:00:00.000Z");
    date.setUTCFullYear(10_000);
