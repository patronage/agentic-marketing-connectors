...
export interface HistoricalComparisonDataIssue {
  message: string;
  source: "api" | "warehouse";
}
...
export interface SourceComparisonArtifact {
  ...
  dataIssues: HistoricalComparisonDataIssue[];
...
export function buildHistoricalComparisonArtifact(
  ...
  const dataIssues: HistoricalComparisonDataIssue[] = [
    ...(input.apiRows.length === 0
      ? [
          {
            message: "Provider comparison rows are empty.",
            source: "api" as const,
          },
        ]
      : []),
    ...(input.warehouseRows.length === 0
      ? [
          {
            message: "Warehouse comparison rows are empty.",
...
export function normalizeWarehouseCampaignDailyRow(row: {
  ...
  date_day?: Date | null | string;
  impressions?: null | number | string;
  spend?: null | number | string;
}): HistoricalComparisonRow {
  const date = warehouseDate(row.date_day);
  if (!row.campaign_id || !date) {
    throw new Error(
      "Warehouse comparison rows require campaign_id and an exact date_day"
    );
  }
  return {
...
function warehouseDate(value: Date | null | string | undefined) {
  if (value instanceof Date) {
    if (Number.isNaN(value.getTime())) {
      return null;
    }
    const isoDate = value.toISOString();
    return /^\d{4}-\d{2}-\d{2}T/u.test(isoDate) ? isoDate.slice(0, 10) : null;
  }
